...
export async function loadIgnorePatterns(file: string): Promise<string[]> {
  ...
  if (!fileContent) {
    ...
    console.warn(
      `Found no "${basename(
        file
      )}" file in the current working directory. Using the default patterns now.`
    );
    console.warn(
      yellow(
        'The default patterns may change over time and it is recommended to provide your own patterns file. You can create a patterns file with "npx mod-pruner init"'
      )
    );
