...
program
  ...
  .option('-c,--config [string]', 'Location of the config file', `./${CONFIG_FILENAME}`)
  .option('-f,--force', 'Update the filesystem', false)
  .option('-cwd,--current-working-directory [string]', 'Set the current working directory of the command', (value: string) => resolve(value), cwd())
